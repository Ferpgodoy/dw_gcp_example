...
CREATE OR REPLACE EXTERNAL TABLE raw.candidates
OPTIONS (
  format = 'CSV',
  uris = ['gs://projeto_1_dw_raw/eleicoes/consulta_cand/{year}/*.csv'],
  skip_leading_rows = 1,
  field_delimiter = ';',
...
-- Check if the table bronze.candidates exists
...
ELSE
  ...
  DELETE FROM bronze.candidates
  WHERE year_partition = DATE({year}, 1, 1);

  INSERT INTO bronze.candidates
  SELECT
    *,
    DATE({year}, 1, 1) AS year_partition,
    CURRENT_TIMESTAMP() AS insert_datetime
